...
class HDC1080:
    def __init__(self, i2c, slave_addr=64):
        ...
        Keyword arguments:
        i2c -- The i2c object (driver) used to interact through
            device addresses.
        slave_addr -- The slave address of the sensor
            (default 64 or 0x40).
        """
        # Perform a scan and make sure we find the slave device
        devices = i2c.scan()
        assert slave_addr in devices, \
            "Did not find slave %d in scan: %s" % (slave_addr, devices)
        self.i2c = i2c
        self.addr = slave_addr
        # received data from temperature and humidity registers
        # is two unsigned characters
        self.fmt = '>2B'
        # Sleep for 15 ms to allow the temperature and humidity
        # temperatures to start recording
        # Only serial number registers 0xFB and 0xFF are available at first
        sleep(0.015)
        # set up for 14 bit resolution (in config register)
    ...
    def read_temperature(self, celsius=False):
        ...
        Keyword arguments:
        celsius -- If the data is kept as celsius after reading
            (default False)
        """
        # write to the pointer register,
        # changing it to the temperature register
        data = bytearray([TEMP_REG])
        self.i2c.writeto(self.addr, data)
        # TODO: Waiting for a temperature update here may not be necessary,
        # based on 14-bit resolution
        sleep(0.0635)
        data = self.i2c.readfrom(self.addr, 2)
        value = int.from_bytes(data, "big")
        # per the spec, the conversion to celsius is
        # (value / (2 ** 16)) * 165 - 40
        if celsius:
    ...
    def read_humidity(self):
        ...
        # write to the pointer register,
        # changing it to the humidity register
        data = bytearray([HUMI_REG])
        self.i2c.writeto(self.addr, data)
        # TODO: Waiting for a humidity update here may not be necessary,
        # based on 14-bit resolution
        sleep(0.065)
        data = self.i2c.readfrom(self.addr, 2)
        value = int.from_bytes(data, "big")
